...
async def font_yins(ayiin):
    ...
    if not kontol:
        return await edit_delete(ayiin, "`Ngetik Yang Bener ya maniez...`")

    await ayiin.edit(f"{kontol}")
    await ayiin.reply("Generated by Lumiere-ubot")
...
async def _(ayiin):
    ...
    if not reply:
        ...
        return await edit_delete(ayiin, "`Kasih Font Dong Sayang :/`")
    ...
    await ayiin.reply("Generated by Lumiere-ubot")
    if not yins:
        await ayiin.reply("Ketik yang bener ya maniez!!")
